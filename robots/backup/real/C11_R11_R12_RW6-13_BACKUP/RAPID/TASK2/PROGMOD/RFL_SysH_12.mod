MODULE RFL_SysH_12
    ...
    PROC rSysH_CopyPos()
        ...
        !
        ! Read current position from T_ROB11
        jpCopy:=CJointT(\TaskName:="T_Rob11");
        !
        ! Read current position
        rReadPosTemp;
        !
    ...
    PROC rSysH_GantryToPos()
        ...
        WHILE bSysHGantryToPos=FALSE DO
            ...
            stMsg{1}:="Y Pos = "+NumToStr(jpTemp.extax.eax_b,0)+" mm";
            stMsg{2}:="Z Pos = "+NumToStr(jpTemp.extax.eax_c,0)+" mm";
            !
            ! User Windoew To Pos 
